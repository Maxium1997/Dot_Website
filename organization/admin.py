...
class CoastPatrolCorpsAdmin(admin.ModelAdmin):
    list_display = ['name', 'en_name', 'address', 'landline_phone', 'central_exchange_intercom']


@admin.register(InternalUnit)
class InternalUnitAdmin(admin.ModelAdmin):
    list_display = ['name', 'en_name', 'serial_number', 'director', 'deputy_director1']
    ordering = ['serial_number']


@admin.register(InspectionOffice)
class InspectionOfficeAdmin(admin.ModelAdmin):
    list_display = ['name', 'en_name', 'address', 'landline_phone', 'intercom_phone']
